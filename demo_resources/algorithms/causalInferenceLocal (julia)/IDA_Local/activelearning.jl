...
function remaining_components(G, v, parents)
    ...
    sps = map(x -> mp[x], sps) # double check this
...
# V = collect(1:td.n)
# subsets = Vector{Set{Integer}}()
# push!(subsets, Set{Integer}())
# for elem in V, j in eachindex(subsets)
#     if length(subsets[j]) < k
#         push!(subsets, union(subsets[j], elem))
#     end
# end
...
function optsingle(cc, memo, fmemo) 
    ...
    for v in vertices(G)
        ...
        for parents in cliques(G, inneighbors(G, v))
            ...
            #println("o " * string(parents) * " "  * string(v) * " " * string(sumedges))
            mx = max(mx, sumedges)
        end
        #println("r " * string(v) * " " * string(mx))
        if mn == -1 || mx < mn
            mn = mx
            mni = v
        end
